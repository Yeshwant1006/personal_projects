...
#Step 1 complete
provisionPing server-vm 519381 1
...
##### Step 2 complete #####
provisionPing server-vm 519381 2
...
echo test > /etc/hostname sed -i 's/127.0.0.1.*localhost/127.0.0.1 test.localdomain test localhost/' /etc/hosts hostname test 
...
PASSWORD=$(mkpasswd -m sha-512 cf6f7507ca9949d8) 
...
#Step 3 complete
provisionPing server-vm 519381 3


# Configure Git Settings
git config --global user.name "Yeshwanth Koltur" && git config --global user.email "yeshwanth.kolthuru@gmail.com"
...
# Step 4 complete 
provisionPing server-vm 519381 4
...
provisionPing server-vm 519381 5
